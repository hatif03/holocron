...
function findRepoRoot(): string {
  let dir = process.cwd();
  for (let i = 0; i < 6; i++) {
    if (fs.existsSync(path.join(dir, "scripts", "seed-showcase-owid.mjs"))) return dir;
...
export async function POST() {
  ...
  const { code, stdout, stderr } = await runScript(cwd, "seed-showcase-owid.mjs");

  if (code === 0) {
    const workIdMatch = stdout.match(/Work ID: ([a-f0-9-]+)/i);
    return NextResponse.json({
      ok: true,
      message: "OWID climate-health showcase loaded.",
